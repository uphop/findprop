...
public interface PostcodeAreaRentPriceRepository extends JpaRepository<PostcodeAreaRentPrice, Long> {
    ...
    @Query(value = "select * from postcode_area_rent_price where postcode_area = :postcodeArea and property_type = :propertyType and bedrooms = :bedrooms and source = 'ons' order by published desc;", nativeQuery = true)
